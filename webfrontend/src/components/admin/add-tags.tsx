...
const AddTags = () => {
  ...
  const addTag = useCallback(() => {
    ...
    if (!isLoading) {
      setIsLoading(true);
      fetch("/new_tag_admin", postRequestContent(requestBody)).then(
        (response) => {
          setIsLoading(false);
      ...
      <form onSubmit={(event) => event.preventDefault()}>
        <div className="m-3">
          ...
          <input
            ...
            autoComplete="off"
            value={tagName}
            onChange={(event) => setTagName(event.target.value.slice(0, 25))}
          />
        </div>
        ...
        <div className="ms-3">
          <input
            ...
            type="file"
            accept="image/*"
            className="form-control m-3 img-input"
            onChange={iconUpload}
          />
          <button
        ...
        <div
          className="m-3 alert alert-danger"
          role="alert"
          style={{ display: emptyErrorMessageTriggered ? "block" : "none" }}
        >
          Tag cannot be empty!
        </div>
      </form>
      <button
        ref={addTagAdminButton}
        style={{ display: "none" }}
        id="add-tag-admin-message-button"
        type="button"
        data-bs-toggle="modal"
        data-bs-target="#add-tag-admin-message"
      ></button>
      <div
        className="modal fade"
        id="add-tag-admin-message"
        aria-labelledby="add-tag-admin-label"
        aria-hidden="true"
      >
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h1 className="modal-title fs-5" id="add-tag-admin-label">
                Message
              </h1>
              <button
                type="button"
